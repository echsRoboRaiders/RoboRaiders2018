package org.usfirst.frc.team2676.robot;

public class RobotMap {
	//Drive Systems Map
//	public static final int DriveLeftM = 6; 
	//public static final int DriveLeftS = 7;
	//public static final int DriveRightM = 4;
	//public static final int DriveRightS = 5;
	
	public static final int DriveLeftM = 2;
	//public static final int DriveRightM = 1;
	
	//Solenoids Map
	//public static final int RightMotionSolenoid = 4;
	//public static final int LeftMotionSolenoid = 6;
	//public static final int RightFixedSolenoid = 5;
	//public static final int LeftFixedSolenoid = 7;
}
